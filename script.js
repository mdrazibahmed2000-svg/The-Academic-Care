...
var firebaseConfig = {
    apiKey: "AIzaSyCHMl5grIOPL5NbQnUMDT5y2U_BSacoXh8", // <-- 🔑 REPLACE THIS WITH YOUR REAL API KEY
...
function loadStudentFees(studentId) {
    ...
    database.ref('students/' + studentId + '/fees').on('value', function(snapshot) {
        if (!snapshot.exists()) {
        ...
        chronologicalMonths.forEach(function(month) {
            if (fees.hasOwnProperty(month)) {
                ...
                // 🔑 MINIMAL CHANGE: Keep original li structure, let CSS handle layout
                html += '<li class="fee-item">' + month + ': ' + statusText + dateMethodDisplay + ' ';
                
                // Admin Buttons
                if(status !== 'break') {
                    html += `<button class="break-btn" onclick="markMonthBreak('${studentId}','${month}')">Break</button>`;
                }
                
                if(status !== 'paid' && status !== 'break') {
                    html += `<button class="mark-paid-btn" onclick="markMonthPaid('${studentId}','${month}')">Paid</button>`;
                }
...
function loadStudentDashboard(studentId) {
    ...
    database.ref('students/' + studentId).on('value', function(snapshot) {
        if (!snapshot.exists()) return logout();
